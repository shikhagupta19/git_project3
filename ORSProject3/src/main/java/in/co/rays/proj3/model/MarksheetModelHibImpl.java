...
public class MarksheetModelHibImpl implements MarksheetModelInt {
	 ...
	 */
	  ...
	  StringBuffer hql = new
			  StringBuffer("from MarksheetDTO order by (physics + chemistry + maths) desc" ); 
	  Query query = session.createQuery(hql.toString());
	  if (pageSize > 0) {
	  pageNo = (pageNo - 1) * pageSize; query.setFirstResult(pageNo);
	  query.setMaxResults(pageSize);
	  } list = query.list();
	  } catch
	  (HibernateException e) 
	  { e.printStackTrace(); throw new  ApplicationException("Exception in  marksheet merit list"); } finally {
	  session.close();
	  } return list;
	 ...
	 * 
	 * 
	 * Session session = null; List list = null; try { session =
	 * HibernateDataSource.getSession(); Criteria
	 * cr=session.createCriteria(MarksheetDTO.class);
	 * 
	 * cr.addOrder(Order.desc("(maths)")); if (pageSize > 0) { pageNo = (pageNo -
	 * 1)* pageSize; cr.setFirstResult(pageNo); cr.setMaxResults(pageSize); } list =
	 * cr.list(); } catch (HibernateException e) { e.printStackTrace(); throw new
	 * ApplicationException("Exception in  marksheet merit list"); } finally {
